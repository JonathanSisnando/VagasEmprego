...
const USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) VagasManausHoje/1.0";
...
async function buscarListaDeVagasSetemp(): Promise<VagaSetempResumo[]> {
  ...
  if (!resposta.ok) throw new Error("Erro ao buscar lista de vagas do Portal do Trabalhador");

  const html = await resposta.text();
  return extrairListaDeVagas(html);
}

function extrairListaDeVagas(html: string): VagaSetempResumo[] {
  const regexItem =
...
function extrairConteudoDaVaga(html: string): ConteudoVagaSetemp | null {
  ...
  for (const linha of linhas) {
    ...
    if (mEsc) {
      escolaridade = limparTexto(mEsc[1]);
      continue;
    }

    const mSal = linha.match(/^Sal[aá]rio\s*:?\s*(.+)$/i);
    if (mSal) {
      salario = limparTexto(mSal[1]);
      continue;
    }

    const mBen = linha.match(/^Benef[ií]cios?\s*:?\s*(.+)$/i);
    if (mBen) {
      beneficios.push(limparTexto(mBen[1]));
      continue;
    }
...
export async function getVagasSetemp(): Promise<FonteResposta> {
